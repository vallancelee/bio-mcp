...
class BioMCPResourceManager:
    ...
    async def get_corpus_status(self) -> ResourceResult:
        ...
        try:
            ...
            try:
                ...
                sync_watermarks: list[dict[str, Any]] = []  # This would come from database query
            except Exception:
                sync_watermarks = []
            ...
            corpus_status = {
                ...
                "system_info": {
                    ...
                    "database_status": "Connected" if self.document_service._initialized else "Disconnected",
    ...
    async def get_corpus_checkpoints(self) -> ResourceResult:
        ...
        try:
            # Get all checkpoints
            checkpoints = await self.checkpoint_service.list_checkpoints(limit=100, offset=0)
            ...
            logger.info("Corpus checkpoints retrieved", 
                       count=len(checkpoint_summaries), execution_time_ms=execution_time)
    ...
    async def get_recent_sync_activities(self) -> ResourceResult:
        ...
        try:
            ...
            logger.info("Recent sync activities retrieved", execution_time_ms=execution_time)
    ...
    async def get_system_health(self) -> ResourceResult:
        ...
        try:
            health_status = {
                ...
                "components": {
                    "database": {
                        "status": "Connected" if self.document_service._initialized else "Disconnected",
                        "last_check": time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime())
                    },
                    "checkpoint_service": {
                        "status": "Connected" if self.checkpoint_service._initialized else "Disconnected",
                        "last_check": time.strftime("%Y-%m-%d %H:%M:%S UTC", time.gmtime())
                    },
                    "pubmed_api": {
