...
class CreateJobRequest(BaseModel):
    ...
    ttl_hours: int = Field(
        default=24, ge=1, le=168, description="Job TTL in hours (1-168)"
    )
...
def create_job_router(job_service_factory=None) -> APIRouter:
    ...
    async def create_job(
        request: CreateJobRequest, job_service: JobService = Depends(get_job_service)
    ) -> CreateJobResponse:
    ...
    async def get_job_status(
        ...
        return JobStatusResponse(
            ...
            duration_ms=int(job_data.duration_ms)
            if job_data.duration_ms is not None
            else None,
    ...
    async def cancel_job(
        ...
        if success:
            ...
            return CancelJobResponse(
                cancelled=False, message="Job already completed or failed"
            )
    ...
    async def list_jobs(
        ...
        jobs = await job_service.list_jobs(status=status, limit=limit, offset=offset)
        ...
        return ListJobsResponse(
            ...
            total=len(
                jobs
            ),  # This is a simplification; real impl would get total count
            limit=limit,
            offset=offset,
        )
