...
def create_error_response(
    error_code: str, message: str, trace_id: str, tool: str | None = None
) -> ErrorResponse:
    """Create a standardized error response (legacy compatibility)."""
    return ErrorResponse(
        error_code=error_code, message=message, trace_id=trace_id, tool=tool
    )
        ...
        with TraceContext(trace_id, request.tool) as trace:
            ...
            try:
                ...
                return InvokeResponse(
                    tool=request.tool, result=result, trace_id=trace_id
                )
