...
class PubMedNormalizer:
    ...
    @staticmethod
    def from_raw_dict(
        raw: dict[str, Any], *, s3_raw_uri: str, content_hash: str
    ) -> Document:
        """
    ...
    def from_dataclass(
        ...
        *,
        ...
        return PubMedNormalizer.from_raw_dict(
            raw_dict, s3_raw_uri=s3_raw_uri, content_hash=content_hash
        )
    ...
    def _extract_authors(raw: dict[str, Any]) -> list[str] | None:
        ...
        if isinstance(authors_raw, str):
            ...
            while i < len(parts):
                ...
                if (
                    i + 1 < len(parts)
                    and len(parts[i + 1].split())
                    <= 2  # First name + optional middle initial
                    and not (
                        i + 2 < len(parts)
                        and parts[i + 2]
                        and parts[i + 2][0].islower()
                    )
                ):  # Next isn't lowercase continuation
    ...
    def _build_detail_fields(raw: dict[str, Any]) -> dict[str, Any]:
        ...
        if mesh_terms:
            if isinstance(mesh_terms, list):
                detail["mesh_terms"] = [
                    term.strip() for term in mesh_terms if term.strip()
                ]
            elif isinstance(mesh_terms, str):
                detail["mesh_terms"] = [
                    term.strip() for term in mesh_terms.split(",") if term.strip()
                ]
...
def to_document(raw: dict[str, Any], *, s3_raw_uri: str, content_hash: str) -> Document:
    ...
    """
    return PubMedNormalizer.from_raw_dict(
        raw, s3_raw_uri=s3_raw_uri, content_hash=content_hash
    )
