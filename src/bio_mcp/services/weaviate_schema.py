...
class WeaviateSchemaManager:
    ...
    def __init__(
        self, client: weaviate.WeaviateClient, config: CollectionConfig = None
    ):
        self.client = client
    ...
    async def create_document_chunk_v2_collection(self) -> bool:
        ...
        logger.info(
            "Creating DocumentChunk_v2 collection",
                        ...
                        nested_properties=[
                            Property(
                                ...
                                nested_properties=[
                                    ...
                                    Property(
                                        name="mesh_terms", data_type=DataType.TEXT_ARRAY
                                    ),
                                    Property(
                                        name="quality_total", data_type=DataType.NUMBER
                                    ),
                                ],
                            )
                        ],
    ...
    def _build_vectorizer_config(self) -> dict[str, Any]:
        ...
        if self.config.vectorizer_type == VectorizerType.OPENAI:
            ...
            return Configure.Vectors.text2vec_openai(model="text-embedding-3-small")

        else:
            raise ValueError(
                f"Unsupported vectorizer type: {self.config.vectorizer_type}"
            )
    ...
    def validate_collection_schema(self, collection_name: str) -> dict[str, Any]:
        ...
        validation_result = {"valid": False, "issues": [], "properties_found": []}
...
class CollectionMigration:
    ...
    async def migrate_from_old_collection(
        self, old_collection: str, new_collection: str, batch_size: int = 100
    ) -> dict[str, Any]:
                ...
                try:
                    ...
                    for obj in batch_result.objects:
                        try:
                            ...
                            new_col.data.insert(
                                uuid=obj.uuid, properties=new_properties
                            )
                            ...
                            migration_stats["errors"].append(
                                f"Failed to migrate {obj.uuid}: {e!s}"
                            )
