...
class ReingestionStats:
    ...
    def add_failure(
        self, doc_uid: str, error: str, context: dict[str, Any] | None = None
    ):
    ...
    def get_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "success_rate": self.documents_processed / total_docs
            if total_docs > 0
            else 0,
            "chunks_created": self.chunks_created,
            "chunks_updated": self.chunks_updated,
            "bytes_processed": self.bytes_processed,
            "docs_per_minute": (total_docs / elapsed.total_seconds() * 60)
...
class ReingestionService:
    ...
    def __init__(self, config: Config):
        ...
        self.retry_attempts = int(
            getattr(config, "BIO_MCP_REINGEST_RETRY_ATTEMPTS", "3")
        )
    ...
    async def start_reingest_job(
        ...
        job_params = {
            ...
            "date_filter": [
                d.isoformat() if d else None for d in (date_filter or (None, None))
            ],
            "pmid_list": pmid_list,
            "dry_run": dry_run,
        ...
        job_id = await self.db_service.create_job(
            operation="reingest", parameters=job_params, status=JobStatus.PENDING
        )
    ...
    async def execute_reingest_job(self, job_id: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not self.config.openai_api_key:
                logger.warning(
                    "No OpenAI API key configured - embeddings will use fallback"
                )
            ...
            for i in range(0, len(document_refs), self.batch_size):
                ...
                logger.info(
                    f"Processed batch {i // self.batch_size + 1}, progress: {progress}%"
                )
            ...
            await self.db_service.update_job_status(
                job_id, final_status, result=final_stats
            )
            ...
            await self.db_service.update_job_status(
                job_id, JobStatus.FAILED, result=stats.get_summary()
            )
            raise
    ...
    async def _process_single_document(
        self, doc_ref: dict[str, Any], stats: ReingestionStats, dry_run: bool
    ):
        ...
        for attempt in range(self.retry_attempts):
            try:
                ...
                if dry_run:
                    ...
                    chunk_uuids = (
                        await self.document_chunk_service.store_document_chunks(
                            document=document,
                            quality_score=0.5,  # Default quality score for testing
                        )
                    )
                    stats.add_success(
                        doc_uid, len(chunk_uuids), len(json.dumps(raw_data))
                    )
                ...
                if attempt == self.retry_attempts - 1:
                    ...
                    stats.add_failure(
                        doc_uid,
                        str(e),
                        {"s3_key": doc_ref.get("s3_key"), "attempts": attempt + 1},
                    )
                    logger.error(
                        f"Failed to process document {doc_uid} after {attempt + 1} attempts: {e}"
                    )
                else:
                    # Retry
                    logger.warning(
                        f"Retrying document {doc_uid} (attempt {attempt + 1}): {e}"
                    )
    ...
    async def _get_document_list(
        ...
        if mode == ReingestionMode.FULL:
            ...
            return await self.db_service.get_document_refs_since(
                since_date, source_filter
            )
    ...
    async def _update_job_progress(
        self, job_id: str, progress: int, stats: ReingestionStats
    ):
        """Update job progress and statistics."""
        await self.db_service.update_job_progress(job_id, progress, stats.get_summary())
