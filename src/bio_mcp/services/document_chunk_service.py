    ...
    async def connect(self) -> None:
        ...
        try:
            ...
            if not self.weaviate_client.client.collections.exists(self.collection_name):
                ...
                success = await self.schema_manager.create_document_chunk_v2_collection()
                if not success:
    ...
    def _convert_filters_to_weaviate(self, filters: dict) -> list[Filter]:
        ...
        for field, value in filters.items():
            if isinstance(value, str | int | float):
                ...
                field_conditions = [
                    Filter.by_property(field).equal(v) for v in value
                ]
                ...
                # Range filters
                if "gte" in value:
                    conditions.append(Filter.by_property(field).greater_or_equal(value["gte"]))
                if "gt" in value:
                    conditions.append(Filter.by_property(field).greater_than(value["gt"]))
                if "lte" in value:
                    conditions.append(Filter.by_property(field).less_or_equal(value["lte"]))
                if "lt" in value:
                    ...
                    if "src" in meta:
                        for source_key, source_data in list(meta["src"].items()):
                            if isinstance(source_data, dict):
                                ...
                                if "provenance" not in cleaned_source or not cleaned_source.get("provenance"):
                                    cleaned_source["provenance"] = {"ingestion_source": "bio-mcp-v2"}
        ...
        try:
            ...
            try:
                ...
                if hasattr(result, 'successful') and hasattr(result, 'objects'):
                    chunk_count = len(result.objects) if result.objects else result.successful
                else:
                ...
                try:
                    ...
                    logger.info(f"Deleted {individual_deletes} chunks individually for document {parent_uid}")
        ...
        try:
            ...
            for group in source_response.groups:
                ...
                try:
                    ...
                    logger.warning(f"Failed to parse group: {e}, group: {group.grouped_by}")
