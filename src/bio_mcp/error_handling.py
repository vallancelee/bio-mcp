...
def validate_tool_arguments(tool_name: str, arguments: dict[str, Any], schema: dict[str, Any]):
    ...
    for field in required:
        if field not in arguments:
            raise ValidationError(
                ...
                {"tool": tool_name, "missing_field": field, "provided_fields": list(arguments.keys())}
            )
            ...
            if expected_type == "string" and not isinstance(value, str):
                if value is not None:  # Allow None for optional fields
                    raise ValidationError(
                        ...
                        {"tool": tool_name, "field": field, "expected": "string", "got": type(value).__name__}
                    )