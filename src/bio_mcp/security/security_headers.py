...
class SecurityValidator:
    ...
    def validate_input(
        self, field_name: str, value: Any, input_type: str = "string"
    ) -> None:
        """
        ...
        if input_type == "string":
            ...
            logger.warning(
                "Unknown input type for validation", field=field_name, type=input_type
            )
...
def validate_tool_input(tool_name: str, arguments: dict[str, Any]) -> RequestContext:
    ...
    logger.debug(
        "Starting security validation", tool=tool_name, arg_count=len(arguments)
    )

    # Tool-specific validation rules
    validation_rules = {
        "pubmed.search": {"term": "query", "limit": "number", "offset": "number"},
        "pubmed.get": {"pmid": "pmid"},
        "pubmed.sync": {"query": "query", "limit": "number"},
        "pubmed.sync.incremental": {"query": "query", "limit": "number"},
        "rag.search": {
            "query": "query",
            "top_k": "number",
            "search_mode": "string",
        ...
        "corpus.checkpoint.get": {"checkpoint_id": "string"},
        "corpus.checkpoint.list": {"limit": "number", "offset": "number"},
        "corpus.checkpoint.delete": {"checkpoint_id": "string"},
    }
    ...
    logger.debug(
        "Security validation completed", tool=tool_name, context_id=context.request_id
    )
    return context
