...
class ErrorClassifier:
    ...
    def classify_error(self, node_result: NodeResult) -> tuple[ErrorType, ErrorSeverity]:
        ...
        """
        error_msg = node_result.error_message.lower() if node_result.error_message else ""
...
class ErrorRecoveryManager:
    ...
    def __init__(self, config: OrchestratorConfig, max_retries: int = 3, base_retry_delay: float = 1.0):
    ...
    async def execute_recovery(
        ...
        updated_state = OrchestratorState(
            ...
            routing_decision=state["routing_decision"] if isinstance(state, dict) else state.routing_decision,
            pubmed_results=state["pubmed_results"] if isinstance(state, dict) else state.pubmed_results,
            ctgov_results=state.get("ctgov_results") if isinstance(state, dict) else getattr(state, 'ctgov_results', None),
            rag_results=state.get("rag_results") if isinstance(state, dict) else getattr(state, 'rag_results', None),
            tool_calls_made=list(state["tool_calls_made"]) if isinstance(state, dict) else list(state.tool_calls_made),
            cache_hits=dict(state["cache_hits"]) if isinstance(state, dict) else dict(state.cache_hits),
            latencies=dict(state["latencies"]) if isinstance(state, dict) else dict(state.latencies),
            node_path=list(state["node_path"]) if isinstance(state, dict) else list(state.node_path),
            messages=list(state["messages"]) if isinstance(state, dict) else list(state.messages),
            errors=list(state["errors"]) + [error_record] if isinstance(state, dict) else list(state.errors) + [error_record],
