...
class BudgetTracker:
    ...
    def get_usage_percentages(self) -> Dict[ResourceType, float]:
        ...
        """
        return {
            ResourceType.TIME: self.time_spent_ms / self.time_budget_ms if self.time_budget_ms > 0 else 1.0,
            ResourceType.TOKENS: self.tokens_used / self.token_budget if self.token_budget > 0 else 1.0,
            ResourceType.REQUESTS: self.requests_made / self.request_budget if self.request_budget > 0 else 1.0
        }
...
class BudgetManager:
    ...
    async def check_budget_status(self, tracker: BudgetTracker) -> tuple[BudgetStatus, str]:
        ...
        if tracker.status == BudgetStatus.ACTIVE:
            ...
            return tracker.status, f"Budget warning: {max_usage*100:.0f}% of resources consumed"
            ...
            return tracker.status, "Token budget exceeded - cannot make more expensive operations"
        
        elif tracker.status == BudgetStatus.REQUEST_EXCEEDED:
            return tracker.status, "Request budget exceeded - cannot make more API calls"
    ...
    async def calculate_timeout(self, tracker: BudgetTracker) -> int:
        ...
        """
        if tracker.status in [BudgetStatus.TIME_EXCEEDED, BudgetStatus.TOKEN_EXCEEDED, BudgetStatus.REQUEST_EXCEEDED]:
            return 0
    ...
    async def estimate_operation_cost(
        ...
        base_costs = operation_costs.get(operation, {
        ...
        if "limit" in args:
            ...
            estimated_costs[ResourceType.TIME] = int(estimated_costs[ResourceType.TIME] * scale_factor)
            estimated_costs[ResourceType.TOKENS] = int(estimated_costs[ResourceType.TOKENS] * scale_factor)
