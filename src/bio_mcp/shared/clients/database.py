...
class CorpusCheckpoint(Base):
    ...
    sync_watermarks = Column(
        JSON, nullable=True, default=dict
    )  # Query -> watermark mapping
...
class DatabaseManager:
    ...
    async def create_document(self, doc_data: dict[str, Any]) -> PubMedDocument:
        ...
        try:
            ...
            logger.error(
                "Document creation failed - integrity error",
                pmid=doc_data.get("pmid"),
                error=str(e),
            )
            raise ValidationError(
                f"Document with PMID {doc_data.get('pmid')} already exists"
            )
        except Exception as e:
            logger.error(
                "Document creation failed", pmid=doc_data.get("pmid"), error=str(e)
            )
            raise
    ...
    async def update_document(
        self, pmid: str, updates: dict[str, Any]
    ) -> PubMedDocument | None:
        ...
        try:
            async with self.get_session() as session:
                result = await session.execute(
                    text(
                        "SELECT * FROM pubmed_documents ORDER BY created_at DESC LIMIT :limit OFFSET :offset"
                    ),
    ...
    async def search_documents_by_title(self, search_term: str) -> list[PubMedDocument]:
        ...
        try:
            async with self.get_session() as session:
                result = await session.execute(
                    text(
                        "SELECT * FROM pubmed_documents WHERE title ILIKE :term ORDER BY created_at DESC"
                    ),
                    {"term": f"%{search_term}%"},
                )
                ...
                logger.info(
                    "Documents found by title search",
    ...
    async def bulk_create_documents(
        self, docs_data: list[dict[str, Any]]
    ) -> list[PubMedDocument]:
        ...
        try:
            ...
            logger.error(
                "Failed to bulk create documents", count=len(docs_data), error=str(e)
            )
            raise
    ...
    async def document_exists(self, pmid: str) -> bool:
        ...
        try:
            async with self.get_session() as session:
                ...
                logger.debug(
                    "Document existence check completed", pmid=pmid, exists=exists
                )
                return exists
    ...
    async def get_sync_watermark(self, query_key: str) -> SyncWatermark | None:
        ...
        try:
            ...
            logger.error(
                "Failed to get sync watermark", query_key=query_key, error=str(e)
            )
            raise
    ...
    async def create_or_update_sync_watermark(
        ...
        logger.debug(
            "Creating/updating sync watermark", query_key=query_key, last_edat=last_edat
        )
        ...
        try:
            async with self.get_session() as session:
                ...
                if existing:
                    ...
                    result = await session.execute(
                        text(
                            "SELECT * FROM sync_watermarks WHERE query_key = :query_key"
                        ),
                        {"query_key": query_key},
                    )
                    row = result.fetchone()
            ...
            logger.error(
                "Failed to create/update sync watermark",
                query_key=query_key,
                error=str(e),
            )
            raise
    ...
    async def create_corpus_checkpoint(
        ...
        try:
            async with self.get_session() as session:
                ...
                result = await session.execute(
                    text(
                        "SELECT checkpoint_id FROM corpus_checkpoints WHERE checkpoint_id = :checkpoint_id"
                    ),
                    {"checkpoint_id": checkpoint_id},
                )
                ...
                watermarks_result = await session.execute(
                    text(
                        "SELECT query_key, last_edat, total_synced FROM sync_watermarks"
                    )
                )
                sync_watermarks = {
                    row.query_key: {
                ...
                if sync_watermarks:
                    edats = [
                        w["last_edat"]
                        for w in sync_watermarks.values()
                        if w["last_edat"]
                    ]
                    if edats:
                ...
                await session.execute(
                    ...
                    """),
                    {
                        ...
                        "primary_queries": json.dumps(checkpoint.primary_queries),
                        "sync_watermarks": json.dumps(checkpoint.sync_watermarks),
            ...
            logger.error(
                "Failed to create corpus checkpoint",
                checkpoint_id=checkpoint_id,
                error=str(e),
            )
            raise

    async def get_corpus_checkpoint(
        self, checkpoint_id: str
    ) -> CorpusCheckpoint | None:
        ...
        try:
            async with self.get_session() as session:
                result = await session.execute(
                    text(
                        "SELECT * FROM corpus_checkpoints WHERE checkpoint_id = :checkpoint_id"
                    ),
                    {"checkpoint_id": checkpoint_id},
                )
                ...
                if row:
                    checkpoint = CorpusCheckpoint(
                        ...
                        if isinstance(row.primary_queries, list)
                        else (
                            json.loads(row.primary_queries)
                            if row.primary_queries
                            else []
                        ),
                        sync_watermarks=row.sync_watermarks
                        if isinstance(row.sync_watermarks, dict)
                        else (
                            json.loads(row.sync_watermarks)
                            if row.sync_watermarks
                            else {}
                        ),
    ...
    async def list_corpus_checkpoints(
        self, limit: int = 50, offset: int = 0
    ...
    async def delete_corpus_checkpoint(self, checkpoint_id: str) -> bool:
        ...
        try:
            async with self.get_session() as session:
                result = await session.execute(
                    text(
                        "DELETE FROM corpus_checkpoints WHERE checkpoint_id = :checkpoint_id"
                    ),
                    {"checkpoint_id": checkpoint_id},
                )
                ...
                if deleted:
                    logger.info(
                        "Corpus checkpoint deleted", checkpoint_id=checkpoint_id
                    )
                else:
                    logger.warning(
                        "Corpus checkpoint not found for deletion",
                        checkpoint_id=checkpoint_id,
                    )
...
class DatabaseHealthCheck:
    ...
    async def check_health(self) -> dict[str, Any]:
        ...
        health_result = {"status": "healthy", "checks": {}}
...
def create_database_engine(config: DatabaseConfig) -> AsyncEngine:
    ...
    engine_kwargs = {"echo": config.echo, "future": True}
