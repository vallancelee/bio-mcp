...
class ClinicalTrialsSyncStrategy(BaseSyncStrategy):
    ...
    async def sync_incremental(
        ...
        try:
            ...
            logger.info(f"Found {len(nct_ids)} trials to sync in {search_duration:.2f}s")
            ...
            if nct_ids:
                ...
                trials_per_second = len(documents) / total_duration if total_duration > 0 else 0
                ...
                return {
                    ...
                    "investment_relevant_count": quality_metrics["investment_relevant_count"],
    ...
    async def _fetch_trials_in_batches(
        ...
        """
        all_trial_data = []
        total_batches = len(nct_ids) // batch_size + (1 if len(nct_ids) % batch_size > 0 else 0)
        
        for i in range(0, len(nct_ids), batch_size):
            batch_ids = nct_ids[i:i + batch_size]
            batch_num = i // batch_size + 1
            
            logger.debug(f"Fetching batch {batch_num}/{total_batches} ({len(batch_ids)} trials)")
            ...
            try:
                ...
                if total_batches > 5 and batch_num % max(1, total_batches // 10) == 0:
                    logger.info(f"Fetch progress: {batch_num}/{total_batches} batches completed")
        ...
        for i, api_data in enumerate(trial_data):
            try:
                ...
                if len(trial_data) > 100 and (i + 1) % 50 == 0:
                    logger.debug(f"Parse progress: {i + 1}/{len(trial_data)} trials processed")
    ...
    async def sync_with_quality_filtering(
        ...
        estimated_filtered_count = int(
            total_trials * max(0, (avg_score - min_quality_score) / (1.0 - min_quality_score))
        ) if avg_score > min_quality_score else 0
        ...
        sync_result.update({
            ...
            "quality_filter_efficiency": estimated_filtered_count / total_trials if total_trials > 0 else 0,
        })
