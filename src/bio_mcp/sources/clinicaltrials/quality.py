...
class ClinicalTrialQualityConfig:
    ...
    HIGH_VALUE_CONDITIONS: frozenset[str] = frozenset([
        ...
        "cancer", "oncology", "tumor", "carcinoma", "leukemia", "lymphoma",
        "melanoma", "sarcoma", "myeloma", "glioma", "adenocarcinoma",
        
        # Metabolic diseases
        "diabetes", "obesity", "metabolic syndrome",
        
        # Neurological diseases  
        "alzheimer", "parkinson", "multiple sclerosis", "als", "huntington",
        "epilepsy", "migraine", "depression", "schizophrenia",
        
        # Autoimmune/inflammatory
        "rheumatoid arthritis", "crohn", "psoriasis", "lupus", "inflammatory bowel",
        
        # Cardiovascular
        "cardiovascular", "heart disease", "heart failure", "hypertension",
    ...
    INVESTMENT_INTERVENTIONS: frozenset[str] = frozenset([
        ...
        "monoclonal antibody", "biological", "biologic", "antibody",
        "immunotherapy", "car-t", "cell therapy", "gene therapy",
        
        # Novel drug classes
        "small molecule", "targeted therapy", "precision medicine",
        
        # Emerging technologies
        "mrna", "vaccine", "genetic", "crispr", "gene editing",
        
        # Medical devices (moderate value)
        "device", "implant", "diagnostic",
...
def calculate_clinical_trial_quality(
    ...
    config: ClinicalTrialQualityConfig | None = None
) -> float:
    """
    ...
    logger.debug(f"Enrollment boost: {enrollment_boost} (count: {trial.enrollment_count})")
    ...
    logger.debug(f"Condition boost: {condition_boost} (conditions: {len(trial.conditions)})")
    ...
    logger.debug(f"Intervention boost: {intervention_boost} (interventions: {len(trial.interventions)})")
...
def _calculate_phase_score(trial: ClinicalTrialDocument, config: ClinicalTrialQualityConfig) -> float:
...
def _calculate_sponsor_score(trial: ClinicalTrialDocument, config: ClinicalTrialQualityConfig) -> float:
...
def _calculate_enrollment_score(trial: ClinicalTrialDocument, config: ClinicalTrialQualityConfig) -> float:
...
def _calculate_status_score(trial: ClinicalTrialDocument, config: ClinicalTrialQualityConfig) -> float:
...
def _calculate_results_score(trial: ClinicalTrialDocument, config: ClinicalTrialQualityConfig) -> float:
    """Calculate results availability quality boost."""
    return config.RESULTS_AVAILABLE_BOOST if trial.has_results else 0.0


def _calculate_condition_score(trial: ClinicalTrialDocument, config: ClinicalTrialQualityConfig) -> float:
...
def _calculate_intervention_score(trial: ClinicalTrialDocument, config: ClinicalTrialQualityConfig) -> float:
