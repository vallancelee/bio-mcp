...
class PubMedService(BaseSourceService[PubMedDocument]):
    ...
    def __init__(self, config: PubMedConfig | None = None, checkpoint_manager: CheckpointManager | None = None):
    ...
    async def initialize(self) -> None:
        ...
        if self.checkpoint_manager:
            self.sync_strategy = PubMedSyncStrategy(self.checkpoint_manager, self.client)
    ...
    async def sync_documents(self, query: str, query_key: str, limit: int) -> dict[str, Any]:
        ...
        if not self.sync_strategy:
            raise RuntimeError("Sync strategy not initialized - checkpoint manager required")
