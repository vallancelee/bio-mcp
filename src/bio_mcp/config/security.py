...
class SecurityValidator:
    ...
    def validate_payload_size(self, data: Any) -> None:
        ...
        try:
            ...
            logger.debug(
                "Payload size validation passed",
                size=size,
    ...
    def validate_object_structure(
        self, obj: Any, depth: int = 0, path: str = "root"
    ) -> Any:
        ...
        if isinstance(obj, dict):
            ...
            if len(obj) > self.config.max_object_keys:
                raise ValidationError(
                    f"Object at '{path}' has {len(obj)} keys, exceeds maximum {self.config.max_object_keys}",
                    {
                        "path": path,
                        "key_count": len(obj),
                        "max_keys": self.config.max_object_keys,
                    },
                )
            ...
            for key, value in obj.items():
                ...
                validated[sanitized_key] = self.validate_object_structure(
                    value, depth + 1, new_path
                )
            ...
            for i, item in enumerate(obj):
                item_path = f"{path}[{i}]"
                validated.append(
                    self.validate_object_structure(item, depth + 1, item_path)
                )
    ...
    def validate_tool_request(
        ...
        logger.debug(
            ...
            tool=tool_name,
            config=self.config.security_level,
        )
        ...
        sanitized_arguments = self.validate_object_structure(
            arguments, path="arguments"
        )

        logger.info("Security validation passed", tool=sanitized_tool_name)

        return {"tool_name": sanitized_tool_name, "arguments": sanitized_arguments}
