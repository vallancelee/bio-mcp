...
class MockJobService:
    ...
    async def create_job(self, tool_name: str, parameters: dict, trace_id: str, ttl_hours: int = 24) -> str:
        """Mock job creation."""
    ...
    async def list_jobs(self, status: JobStatus = None, limit: int = 50, offset: int = 0) -> list[JobData]:
        """Mock job listing."""
...
class TestJobAPI:
    ...
    def test_list_jobs_with_pagination(self, app_client, mock_job_service):
        ...
        assert len(data["jobs"]) == 3
        assert data["total"] == 3  # API currently returns count of returned jobs, not total available
        assert data["limit"] == 3
