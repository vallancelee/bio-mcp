...
class TestJobWorker:
    ...
    @pytest.mark.asyncio
    async def test_execute_single_job_success(self, job_worker, mock_tool_executor, mock_job_service):
    ...
    @pytest.mark.asyncio
    async def test_execute_single_job_failure(self, job_worker, mock_tool_executor, mock_job_service):
    ...
    @pytest.mark.asyncio
    async def test_worker_concurrency_limit(self, job_worker, mock_tool_executor, mock_job_service):
        ...
        completed_jobs = [u for u in mock_job_service.status_updates if u[0] == "complete"]
        assert len(completed_jobs) >= 3  # At least 3 should be processed
        assert len(job_worker.running_jobs) == 0  # All should be cleaned up
    
    @pytest.mark.asyncio
    async def test_worker_job_polling(self, job_worker, mock_tool_executor, mock_job_service):
    ...
    @pytest.mark.asyncio
    async def test_worker_graceful_shutdown(self, job_worker, mock_tool_executor, mock_job_service):
    ...
    @pytest.mark.asyncio
    async def test_worker_error_handling(self, job_worker, mock_tool_executor, mock_job_service):
