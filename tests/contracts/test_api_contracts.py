...
class TestAPIContracts:
        ...
        try:
            result = await asyncio.wait_for(
                rag_tools.rag_search_tool(
                    "rag.search",
                    {"query": "cancer treatment", "top_k": 5, "quality_bias": True},
                ),
    ...
    def _validate_rag_search_response(self, response: dict[str, Any]):
        ...
        assert "execution_time_ms" in metadata, (
            "Missing 'execution_time_ms' in metadata"
        )
        ...
        for field in optional_number_fields:
            if field in item:
                assert item[field] is None or isinstance(item[field], int | float), (
                    f"{field} must be number or null"
                )

        # Optional object/string fields
        if "explain_score" in item:
            assert isinstance(item["explain_score"], str | dict), (
                "explain_score must be string or object"
            )
    ...
    async def test_rag_get_response_schema(self, sample_documents):
        ...
        try:
            result = await asyncio.wait_for(
                rag_tools.rag_get_tool("rag.get", {"doc_id": "pmid:12345678"}),
                timeout=10.0,
            )
        except TimeoutError:
    ...
    def _validate_rag_get_response(self, response: dict[str, Any]):
        ...
        if response.get("success"):
            ...
            assert data["title"] is None or isinstance(data["title"], str), (
                "title must be string or null"
            )

            # journal can be string or null
            assert data["journal"] is None or isinstance(data["journal"], str), (
                "journal must be string or null"
            )
    ...
    def _validate_quality_object(self, quality: dict[str, Any]):
        ...
        for field in optional_int_fields:
            if field in quality:
                assert quality[field] is None or isinstance(quality[field], int), (
                    f"quality.{field} must be integer or null"
                )
    ...
    async def test_corpus_checkpoint_get_schema(self, sample_checkpoint):
        ...
        result = await corpus_tools.corpus_checkpoint_get_tool(
            "corpus.checkpoint.get", {"checkpoint_id": "test_query"}
        )
    ...
    def _validate_error_envelope(self, error: dict[str, Any]):
        ...
        if "details" in error:
            ...
            assert error["details"] is None or isinstance(
                error["details"], dict | list | str
...
class TestContractStability:
    ...
    async def test_doc_id_pattern_consistency(self, sample_documents):
        ...
        result = await rag_tools.rag_search_tool(
            "rag.search", {"query": "test document", "top_k": 5}
        )
    ...
    async def test_response_time_constraints(self, sample_documents):
        ...
        result = await rag_tools.rag_search_tool(
            "rag.search", {"query": "quick test", "top_k": 5}
        )
    ...
    def test_schema_versioning_compatibility(self):
        ...
        assert rag_search_required_fields == {"results"}, (
            "Breaking change in rag.search response"
        )
        assert rag_get_required_fields == {
            "doc_id",
            "title",
            "journal",
            "pub_types",
            "quality",
            "version",
        }, "Breaking change in rag.get response"
        assert checkpoint_get_required_fields == {"query_key", "last_edat"}, (
            "Breaking change in checkpoint.get response"
        )
        assert checkpoint_set_required_fields == {"ok"}, (
            "Breaking change in checkpoint.set response"
        )
        assert error_envelope_required_fields == {"code", "message"}, (
            "Breaking change in error envelope"
        )
