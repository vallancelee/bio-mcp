...
class TestCoreContracts:
    ...
    def test_document_uid_contract_stability(self, sample_raw_data):
        ...
        document = PubMedNormalizer.from_raw_dict(
            sample_raw_data, s3_raw_uri="s3://test/test.json", content_hash="test123"
        )
    ...
    def test_chunk_id_contract_stability(self, sample_raw_data):
        ...
        # Create test chunks using new format
        chunks = []
        for i in range(3):
            chunk_id = f"s{i}"  # New format: s0, s1, s2
            chunk = Chunk(
                chunk_id=chunk_id,
        ...
        # Contract: Chunk ID must follow "s{n}" format and have deterministic UUIDs
        for i, chunk in enumerate(chunks):
            ...
            expected_uuid = Chunk.generate_uuid(document.uid, chunk.chunk_id)
    ...
    def test_database_model_contract_stability(self, sample_raw_data):
        ...
        db_record = NormalizedDocument.from_document(
            document=document, s3_raw_uri="s3://test/test.json", content_hash="test123"
        )
    ...
    def test_doc_id_pattern_contract_stability(self):
        ...
        for doc_id in valid_doc_ids:
            assert re.match(stable_pattern, doc_id), (
                f"Valid doc_id {doc_id} failed pattern"
            )

        for doc_id in invalid_doc_ids:
            assert not re.match(stable_pattern, doc_id), (
                f"Invalid doc_id {doc_id} passed pattern"
            )
...
class TestSchemaEvolutionSafety:
    ...
    def test_breaking_change_detection(self):
        ...
        assert supports_backward_compatibility is True, (
            "Backward compatibility flag changed"
        )
