...
class TestJSONSchemaContracts:
    ...
    def _validate_against_schema(
        self, data: dict[str, Any], schema: dict[str, Any], schema_name: str
    ):
    ...
    @pytest.mark.asyncio
    async def test_rag_search_schema_compliance(
        self, rag_search_response_schema, sample_documents
    ):
        ...
        result = await rag_search_tool(
            "rag.search", {"query": "cancer treatment", "top_k": 3}
        )
        ...
        self._validate_against_schema(
            response_data, rag_search_response_schema, "rag.search response"
        )

    @pytest.mark.asyncio
    async def test_rag_get_schema_compliance(
        self, rag_get_response_schema, sample_documents
    ):
        """Test that rag.get responses match JSON schema exactly."""
        from bio_mcp.mcp.rag_tools import rag_get_tool

        result = await rag_get_tool("rag.get", {"doc_id": "pmid:12345678"})

        assert len(result) == 1
        response_data = self._extract_json_from_response(result[0].text)

        self._validate_against_schema(
            response_data, rag_get_response_schema, "rag.get response"
        )

    @pytest.mark.asyncio
    async def test_checkpoint_get_schema_compliance(
        self, checkpoint_get_response_schema
    ):
        ...
        result = await corpus_checkpoint_get_tool(
            "corpus.checkpoint.get", {"checkpoint_id": "test_schema_query"}
        )
        ...
        self._validate_against_schema(
            response_data, checkpoint_get_response_schema, "checkpoint.get response"
        )

    @pytest.mark.asyncio
    async def test_checkpoint_create_schema_compliance(
        self, checkpoint_set_response_schema
    ):
    ...
    async def test_error_response_schema_compliance(self, error_envelope_schema):
        ...
        if "```json" in response_text or response_text.strip().startswith("{"):
            ...
            if "error" in response_data:
                self._validate_against_schema(
                    response_data, error_envelope_schema, "error envelope"
                )
...
class TestContractRegression:
    ...
    def test_required_fields_never_removed(self):
        ...
        contracts = {
            ...
            "corpus.checkpoint.get": {
                "required_response_fields": [
                    "query_key",
                    "last_edat",
                ]  # last_edat can be null
            },
            "corpus.checkpoint.set": {"required_response_fields": ["ok"]},
            "error_envelope": {"required_error_fields": ["code", "message"]},
        }
        ...
        assert contracts["rag.get"]["required_response_fields"] == [
            "doc_id",
            "title",
            "journal",
            "pub_types",
        ...
        assert contracts["corpus.checkpoint.get"]["required_response_fields"] == [
            "query_key",
            "last_edat",
        ]
        assert contracts["corpus.checkpoint.set"]["required_response_fields"] == ["ok"]
        assert contracts["error_envelope"]["required_error_fields"] == [
            "code",
            "message",
        ]
    ...
    def test_version_field_constraints(self):
        ...
        for version in valid_versions:
            assert isinstance(version, int) and version >= 1, (
                f"Valid version {version} failed constraint"
            )

        for version in invalid_versions:
            assert not (isinstance(version, int) and version >= 1), (
                f"Invalid version {version} passed constraint"
            )
