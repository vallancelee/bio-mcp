...
class TestDocumentModelCompatibility:
    ...
    def test_document_to_normalized_document_conversion(self, normalized_document):
        ...
        db_record = NormalizedDocument.from_document(
            document=normalized_document, s3_raw_uri=s3_uri, content_hash=content_hash
        )
    ...
    def test_normalized_document_to_api_response_format(self, normalized_document):
        ...
        assert api_dict["published_at"] is None or isinstance(
            api_dict["published_at"], str
        )
    ...
    @pytest.mark.asyncio
    async def test_rag_search_response_consistency_with_documents(
    ...
    def test_chunk_id_format_stability(self, normalized_document):
        ...
        # Chunk ID format must be stable: "w{n}" for window-based chunks
        for i, chunk in enumerate(chunks):
            ...
            expected_uuid = Chunk.generate_uuid(normalized_document.uid, chunk.chunk_id)
    ...
    def _create_test_chunks(self, document: Document) -> list[Chunk]:
        ...
        text_parts = [
            document.text[i : i + 100] for i in range(0, len(document.text), 100)
        ]

        chunks = []
        for i, text_part in enumerate(text_parts):
            chunk_id = f"w{i}"  # Use window-based chunking format
            chunk = Chunk(
                chunk_id=chunk_id,
    ...
    def test_quality_score_compatibility(self):
        ...
        for field in ["design", "recency", "journal", "human"]:
            if field in quality_example:
                assert quality_example[field] is None or isinstance(
                    quality_example[field], int
                )
    ...
    async def test_error_handling_consistency(self):
        ...
        result = await rag_get_tool("rag.get", {"doc_id": "pmid:99999999999"})
    ...
    def test_backward_compatibility_markers(self):
        ...
        assert supports_legacy_pmid_format is True, (
            "Legacy PMID format must be supported"
        )
    ...
    async def test_response_performance_consistency(self, sample_documents):
        ...
        result = await rag_search_tool(
            "rag.search", {"query": "performance test", "top_k": 10}
        )
