...
class TestRAGQuality:
    ...
    async def test_section_boosting(self, rag_tools_manager, test_queries):
        ...
        assert results_found or conclusions_found, (
            "No Results or Conclusions sections found in top results"
        )
        ...
        for doc in results.documents:
            ...
            if title and abstract:
                ...
                assert not abstract.startswith(title), (
                    f"Abstract starts with title: {title[:50]}"
                )

                # Title should be separate from abstract content
                assert title != abstract[: len(title)], (
                    f"Title duplicated in abstract: {title[:30]}"
                )

    @pytest.mark.asyncio
    async def test_document_reconstruction_vs_chunks(
        self, rag_tools_manager, test_queries
    ):
        ...
        chunk_results = await rag_tools_manager.search_documents(
            query=query, top_k=10, return_chunks=True
        )

        # Get reconstructed documents
        doc_results = await rag_tools_manager.search_documents(
            query=query, top_k=10, return_chunks=False
        )
        ...
        if doc_results.documents:
            ...
            assert "sections_found" in first_doc, (
                "Document missing sections_found field"
            )
    ...
    async def test_biomedical_query_enhancement(self, rag_tools_manager, test_queries):
        ...
        results = await rag_tools_manager.search_documents(
            query=query_info["query"], top_k=5, enhance_query=True
        )

        assert results.total_results > 0, (
            f"No results for enhanced COVID query: {query_info['query']}"
        )

        # Performance metadata should indicate query was enhanced
        if hasattr(results, "performance") and results.performance:
            assert results.performance.get("enhanced_query") is True, (
                "Query enhancement not indicated in performance data"
            )
    ...
    async def test_query_enhancement_comparison(self, rag_tools_manager, test_queries):
        ...
        enhanced_results = await rag_tools_manager.search_documents(
            query=query, top_k=5, enhance_query=True, return_chunks=False
        )

        # Search without enhancement
        unenhanced_results = await rag_tools_manager.search_documents(
            query=query, top_k=5, enhance_query=False, return_chunks=False
        )

        # Both should return results
        assert enhanced_results.total_results > 0, (
            f"No enhanced results for query: {query}"
        )
        assert unenhanced_results.total_results > 0, (
            f"No unenhanced results for query: {query}"
        )
        ...
        if hasattr(enhanced_results, "performance") and enhanced_results.performance:
            assert enhanced_results.performance.get("enhanced_query") is True, (
                "Enhanced query not flagged in performance"
            )
        if (
            hasattr(unenhanced_results, "performance")
            and unenhanced_results.performance
        ):
            assert unenhanced_results.performance.get("enhanced_query") is False, (
                "Unenhanced query incorrectly flagged"
            )
    ...
    async def test_section_boost_in_chunks(self, rag_tools_manager, test_queries):
        ...
        results = await rag_tools_manager.search_documents(
            query=query_info["query"], top_k=10, return_chunks=True
        )

        assert results.total_results > 0, (
            f"No chunk results for query: {query_info['query']}"
        )
        ...
        for chunk in results.documents:
            ...
            if "section" in chunk:
                ...
                if section in high_value_sections:
                    ...
                    assert chunk.get("score", 0) > 0, (
                        f"Chunk from {section} section has no score"
                    )
    ...
    async def test_performance_metrics(self, rag_tools_manager):
        ...
        results = await rag_tools_manager.search_documents(
            query="biomedical research", top_k=5, return_chunks=False
        )
        ...
        assert "reconstructed_docs" in results.performance, (
            "Missing reconstructed_docs flag"
        )

        # Reconstructed docs should be True since return_chunks=False
        assert results.performance["reconstructed_docs"] is True, (
            "reconstructed_docs should be True"
        )
    ...
    async def test_abstract_quality(self, rag_tools_manager, test_queries):
        ...
        results = await rag_tools_manager.search_documents(
            query=query_info["query"], top_k=5, return_chunks=False
        )

        assert results.total_results > 0, (
            f"No results for clinical trial query: {query_info['query']}"
        )
        ...
        for doc in results.documents:
            ...
            if abstract:
                ...
                assert len(abstract.split()) > 5, (
                    f"Abstract too short: {len(abstract.split())} words"
                )
                assert "." in abstract or "!" in abstract or "?" in abstract, (
                    "Abstract lacks sentence structure"
                )
                ...
                assert abstract == abstract.strip(), (
                    "Abstract has leading/trailing whitespace"
                )

                # If we have a title, it shouldn't appear verbatim in the abstract
                if title and len(title) > 10:
                    assert not abstract.startswith(title), (
                        f"Abstract starts with title: {title[:30]}"
                    )
