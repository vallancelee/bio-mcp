...
class MockPubMedData:
    ...
    def create_documents(cls, pmids: list[str]) -> list[PubMedDocument]:
        ...
        for pmid in pmids:
            ...
            doc_data = next(
                (doc for doc in cls.SAMPLE_DOCUMENTS if doc["pmid"] == pmid), None
            )
...
class TestSyncOrchestrator:
    ...
    @pytest.mark.asyncio
    async def test_sync_documents_basic_workflow(
        self, database_manager, weaviate_client
    ):
    ...
    @pytest.mark.asyncio
    async def test_sync_documents_with_existing_documents(
        self, database_manager, weaviate_client
    ):
        ...
        print(
            f"✓ Handled existing documents: {result['already_existed']} existing, {result['successfully_synced']} new"
        )

    @pytest.mark.asyncio
    async def test_sync_documents_error_handling(
        self, database_manager, weaviate_client
    ):
        ...
        pubmed_service.search = AsyncMock(return_value=search_result)
        pubmed_service.fetch_documents = AsyncMock(
            side_effect=Exception("PubMed API error")
        )
