...
async def test_end_to_end_synthesis():
    ...
    state = OrchestratorState(
        ...
        errors=[],
        node_path=["parse_frame", "router", "pubmed_search", "ctgov_search", "rag_search"],
        answer=None,
        checkpoint_id=None,
        messages=[]
    )
    ...
    assert synthesis_metrics["answer_type"] in ["comprehensive", "partial"]  # Should be good quality
...
async def test_deterministic_checkpoint_ids():
    ...
    assert checkpoint1_hash == checkpoint2_hash, "Checkpoint IDs should be deterministic for identical inputs"
