...
from bio_mcp.orchestrator.budget.manager import BudgetManager, BudgetStatus, ResourceType
...
class TestM3Integration:
    ...
    async def test_m3_full_workflow_integration(self):
        ...
        budget_status, budget_msg = await budget_manager.check_budget_status(budget_tracker)
        assert budget_status == BudgetStatus.ACTIVE
        
        # Test 2: Simulate successful PubMed operation with budget tracking
        pubmed_operation_cost = await budget_manager.estimate_operation_cost("pubmed.search", {"limit": 20})
        can_afford = await budget_manager.can_afford_operation(budget_tracker, "pubmed.search", {"limit": 20})
        assert can_afford is True
        
        # Consume budget for PubMed search
        success = await budget_manager.enforce_budget(budget_tracker, ResourceType.TIME, pubmed_operation_cost[ResourceType.TIME])
        assert success is True
        success = await budget_manager.enforce_budget(budget_tracker, ResourceType.TOKENS, pubmed_operation_cost[ResourceType.TOKENS])
        assert success is True
        success = await budget_manager.enforce_budget(budget_tracker, ResourceType.REQUESTS, 1)
        assert success is True
        ...
        updated_state_1.update({
            ...
            "latencies": {"pubmed_search": pubmed_operation_cost[ResourceType.TIME]},
            "node_path": ["frame_parser", "router", "enhanced_pubmed"],
            "messages": initial_state["messages"] + [{
                "role": "system", 
                "content": "PubMed search completed: 2 results"
            }]
        })
        ...
        recovery_strategy = await error_manager.create_recovery_strategy(trials_error, attempt=1, max_attempts=3)
        ...
        trials_operation_cost = await budget_manager.estimate_operation_cost("clinicaltrials.search", {"limit": 50})
        can_afford_retry = await budget_manager.can_afford_operation(budget_tracker, "clinicaltrials.search", {"limit": 50})
        assert can_afford_retry is True
        
        # Consume budget for trials retry
        await budget_manager.enforce_budget(budget_tracker, ResourceType.TIME, trials_operation_cost[ResourceType.TIME])
        await budget_manager.enforce_budget(budget_tracker, ResourceType.TOKENS, trials_operation_cost[ResourceType.TOKENS])
        ...
        final_state.update({
            ...
            "tool_calls_made": recovered_state["tool_calls_made"] + ["clinicaltrials.search"],
            "cache_hits": {
                **recovered_state["cache_hits"],
                "trials_search": False
            },
            "latencies": {
        ...
        assert final_checkpoint.partial_results is True  # Marked as partial due to recovered error
        ...
        assert budget_summary["usage"][ResourceType.REQUESTS] == 2  # PubMed + ClinicalTrials
        ...
        cursor.execute("SELECT * FROM query_metrics WHERE checkpoint_id = ?", (checkpoint.checkpoint_id,))
        ...
        print(f"🔬 Results: PubMed={len(final_state['pubmed_results']['results'])}, Trials={len(final_state['ctgov_results']['results'])}")
        print(f"⚠️  Errors recovered: {final_checkpoint.error_count}")
        print(f"💰 Budget used: {budget_summary['usage'][ResourceType.TIME]}ms time, {budget_summary['usage'][ResourceType.TOKENS]} tokens, {budget_summary['usage'][ResourceType.REQUESTS]} requests")
    ...
    async def test_m3_budget_exceeded_scenario(self):
        ...
        pubmed_cost = await budget_manager.estimate_operation_cost("pubmed.search", {"limit": 10})
        can_afford_1 = await budget_manager.can_afford_operation(budget_tracker, "pubmed.search", {"limit": 10})
        assert can_afford_1 is True
        
        # Consume budget for first operation
        await budget_manager.enforce_budget(budget_tracker, ResourceType.TIME, pubmed_cost[ResourceType.TIME])
        await budget_manager.enforce_budget(budget_tracker, ResourceType.TOKENS, pubmed_cost[ResourceType.TOKENS])
        await budget_manager.enforce_budget(budget_tracker, ResourceType.REQUESTS, 1)
        
        # Test 2: Check if expensive operation can be afforded (should fail)
        rag_cost = await budget_manager.estimate_operation_cost("rag.search", {"query": "diabetes", "limit": 20})
        can_afford_expensive = await budget_manager.can_afford_operation(budget_tracker, "rag.search", {"query": "diabetes", "limit": 20})
        assert can_afford_expensive is False  # Should not be able to afford due to token budget
        
        # Test 3: Check budget status (should be warning or exceeded)
        budget_status, budget_msg = await budget_manager.check_budget_status(budget_tracker)
        assert budget_status in [BudgetStatus.ACTIVE, BudgetStatus.WARNING, BudgetStatus.TOKEN_EXCEEDED, BudgetStatus.TIME_EXCEEDED, BudgetStatus.REQUEST_EXCEEDED]
        assert any(word in budget_msg.lower() for word in ["budget", "active", "warning", "exceeded"])
        ...
        partial_state.update({
            ...
            "node_path": ["frame_parser", "router", "enhanced_pubmed"],
            "messages": [{"role": "system", "content": "Partial results due to budget constraints"}],
            "errors": [{
                "node": "budget_manager",
        ...
        print(f"📊 Final checkpoint marked as partial: {final_checkpoint.partial_results}")
    ...
    async def test_m3_error_cascade_handling(self):
        ...
        for attempt, error in enumerate(error_scenarios, 1):
            strategy = await error_manager.create_recovery_strategy(error, attempt, max_attempts=3)
            ...
            # Apply recovery
            current_state = await error_manager.execute_recovery(strategy, current_state, error, "pubmed.search")
            
            # Update checkpoint after each error
            await state_manager.update_checkpoint(checkpoint.checkpoint_id, current_state)
        ...
        recovery_markers = [node for node in current_state["node_path"] if node.startswith("recovery_")]
        assert len(recovery_markers) == 3  # One for each error processed (2 retry + 1 fail_permanently)
        
        # Try a different node that should succeed
        trials_cost = await budget_manager.estimate_operation_cost("clinicaltrials.search", {"limit": 20})
        can_afford = await budget_manager.can_afford_operation(budget_tracker, "clinicaltrials.search", {"limit": 20})
        assert can_afford is True
        
        # Simulate successful trials operation
        await budget_manager.enforce_budget(budget_tracker, ResourceType.TIME, trials_cost[ResourceType.TIME])
        await budget_manager.enforce_budget(budget_tracker, ResourceType.TOKENS, trials_cost[ResourceType.TOKENS])
        await budget_manager.enforce_budget(budget_tracker, ResourceType.REQUESTS, 1)
        
        # Update state with successful trials results
        current_state.update({
            "ctgov_results": {"results": [{"nct_id": "NCT999888"}], "total": 1},
            "tool_calls_made": current_state["tool_calls_made"] + ["clinicaltrials.search"],
            "latencies": {**current_state["latencies"], "trials_search": trials_cost[ResourceType.TIME]},
            "node_path": current_state["node_path"] + ["enhanced_trials"],
            "messages": current_state["messages"] + [{"role": "system", "content": "ClinicalTrials succeeded despite PubMed failures"}]
        })
        ...
        assert final_checkpoint.partial_results is True  # Partial due to PubMed failures
        assert final_checkpoint.completed_at is not None
        ...
        print(f"🎯 ClinicalTrials results: {len(current_state['ctgov_results']['results'])}")
