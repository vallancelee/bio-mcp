...
class TestM2Integration:
    ...
    async def test_m2_full_workflow_integration(self):
        ...
        mock_pubmed_tool.execute = AsyncMock(return_value={
        ...
        async def mock_search_task(term: str) -> NodeResult:
            return NodeResult(
                success=True,
                data={"results": [{"pmid": f"pmid_{term}", "title": f"Article about {term}"}], "total": 1},
                node_name="mock_search"
            )
        
        parallel_tasks = [
            {"func": mock_search_task, "args": ("diabetes",), "kwargs": {}, "token_cost": 1},
            {"func": mock_search_task, "args": ("cancer",), "kwargs": {}, "token_cost": 1},
        ]
        ...
        pubmed_node.adapter.execute_tool = AsyncMock(return_value=NodeResult(
            success=True,
            data={
                "results": [
        ...
        trials_node.adapter.execute_tool = AsyncMock(return_value=NodeResult(
            success=True,
            data={
                "results": [
                    {
        ...
        assert len(final_state["node_path"]) == 4  # frame_parser, router, enhanced_pubmed, enhanced_trials
        assert "pubmed_results" in final_state
        assert "trials_results" in final_state
        assert len(final_state["tool_calls_made"]) >= 2  # At least pubmed.search and clinicaltrials.search
        ...
        print(f"📊 Final state contains {final_state['pubmed_results']['total_results']} PubMed results")
        print(f"🔬 Final state contains {final_state['trials_results']['filtered_count']} quality trials")
    ...
    async def test_m2_error_handling_integration(self):
        ...
        rate_limiter = TokenBucketRateLimiter(capacity=2, refill_rate=1.0)  # Very restrictive
        ...
        async def slow_failing_task(should_fail: bool = False) -> NodeResult:
            ...
            return NodeResult(success=True, data={"status": "ok"}, node_name="test_task")
        
        # Mix of successful and failing tasks
        stress_tasks = [
            {"func": slow_failing_task, "args": (False,), "kwargs": {}, "token_cost": 1},
            {"func": slow_failing_task, "args": (True,), "kwargs": {}, "token_cost": 1},  # This will fail
            {"func": slow_failing_task, "args": (False,), "kwargs": {}, "token_cost": 1},
        ]
    ...
    async def test_m2_performance_characteristics(self):
        ...
        async def concurrency_tracking_task(task_id: str) -> NodeResult:
            ...
            concurrent_counter["max_seen"] = max(concurrent_counter["max_seen"], concurrent_counter["value"])
            
            await asyncio.sleep(0.05)  # Hold the concurrency slot briefly
            
            concurrent_counter["value"] -= 1
            return NodeResult(success=True, data={"task_id": task_id}, node_name="concurrency_test")
        
        concurrency_tasks = [
            {"func": concurrency_tracking_task, "args": (f"task_{i}",), "kwargs": {}, "token_cost": 1}
            for i in range(4)
        ]
        ...
        print(f"🎯 Max concurrency observed: {concurrent_counter['max_seen']} (limit: 2)")
