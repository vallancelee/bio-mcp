...
class TestReingestionIntegration:
    ...
    async def test_job_lifecycle_validation_mode(self, config):
        ...
        with (
            patch.object(
                service.db_service, "create_job", return_value="test-job-id"
            ) as mock_create,
            ...
            patch.object(
                service.document_chunk_service, "connect"
            ) as mock_weaviate_connect,
            patch.object(
                service.document_chunk_service, "disconnect"
            ) as mock_weaviate_disconnect,
            ...
            job_id = await service.start_reingest_job(
                mode=ReingestionMode.VALIDATION, dry_run=True
            )
    ...
    async def test_error_handling_and_retry_logic(self, config):
        ...
        stats.add_failure(
            "test:123", "Timeout error", {"s3_key": "test.json", "attempts": 3}
        )
    ...
    async def test_batch_processing_logic(self, config):
        ...
        with (
            patch.object(service.db_service, "create_job", return_value="test-job-id"),
            patch.object(service.db_service, "get_job") as mock_get_job,
            ...
            patch.object(
                service, "_process_single_document", new_callable=AsyncMock
            ) as mock_process,
            patch.object(
                service, "_update_job_progress", new_callable=AsyncMock
    ...
    async def test_job_cancellation(self, config):
        ...
        with (
            ...
            mock_update_status.assert_called_once_with(
                "test-job-id", JobStatus.CANCELLED
            )
