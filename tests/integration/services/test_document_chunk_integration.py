...
class TestDocumentChunkIntegration:
    ...
    def sample_document(self):
        ...
        return Document(
            ...
            detail={
                ...
                "mesh_terms": ["testing", "integration", "biobert"],
            },
        )
    ...
    async def embedding_service(self, integration_config):
        ...
        service = DocumentChunkService(
            collection_name=integration_config.weaviate_collection_v2
        )
        ...
        try:
            ...
            if service.weaviate_client.client.collections.exists(
                integration_config.weaviate_collection_v2
            ):
                service.weaviate_client.client.collections.delete(
                    integration_config.weaviate_collection_v2
                )
                print(
                    f"Dropped existing test collection: {integration_config.weaviate_collection_v2}"
                )
            ...
            try:
                ...
                await service.delete_document_chunks(
                    "pubmed:99999998"
                )  # High quality test doc
                await service.delete_document_chunks(
                    "pubmed:99999997"
                )  # Low quality test doc
            except Exception:
                pass
    ...
    async def test_full_chunk_pipeline(self, embedding_service, sample_document):
        ...
        chunk_uuids = await embedding_service.store_document_chunks(
            document=sample_document, quality_score=0.9
        )
        ...
        results = await embedding_service.search_chunks(
            query="integration testing biobert", limit=10
        )
        ...
        assert (
            first_chunk["meta"]["src"]["pubmed"]["journal"]
    ...
    async def test_idempotent_storage(self, embedding_service, sample_document):
        ...
        assert chunk_uuids_1 == chunk_uuids_2, (
            "Should generate same UUIDs for same document"
        )

        # Verify no duplicates in search
        results = await embedding_service.search_chunks(
            query=sample_document.title, limit=50
        )

        parent_uid_matches = [
            r for r in results if r["parent_uid"] == sample_document.uid
        ]
    ...
    async def test_search_with_filters(self, embedding_service, sample_document):
        ...
        # Store document
        chunk_uuids = await embedding_service.store_document_chunks(
            document=sample_document, quality_score=0.8
        )
    ...
    async def test_quality_boosting(self, embedding_service, sample_document):
        ...
        high_quality_doc = Document(
            ...
            detail={
                ...
                "mesh_terms": ["research", "biomedical", "quality"],
            },
        )
        ...
        low_quality_doc = Document(
            ...
            detail={
                ...
                "mesh_terms": ["research", "basic", "preliminary"],
            },
        )

        try:
            # Store both documents with different quality scores
            await embedding_service.store_document_chunks(
                high_quality_doc, quality_score=0.9
            )
            await embedding_service.store_document_chunks(
                low_quality_doc, quality_score=0.3
            )

            # Search for content that should match both documents
            results = await embedding_service.search_chunks(
                query="research biomedical quality methodology findings", limit=20
            )

            # Find results from each document
            high_quality_results = [
                r for r in results if r["parent_uid"] == high_quality_doc.uid
            ]
            low_quality_results = [
                r for r in results if r["parent_uid"] == low_quality_doc.uid
            ]
            ...
            if high_quality_results and low_quality_results:
                ...
                print(
                    f"High quality scores: {[r['score'] for r in high_quality_results]}"
                )
                print(
                    f"Low quality scores: {[r['score'] for r in low_quality_results]}"
                )
                print(
                    f"High quality boosts: {[r['quality_boost'] for r in high_quality_results]}"
                )
                print(
                    f"Low quality boosts: {[r['quality_boost'] for r in low_quality_results]}"
                )

                # If both have base_score of 0 (no embeddings), then quality boost should differentiate
                high_quality_boost = max(
                    r["quality_boost"] for r in high_quality_results
                )
                low_quality_boost = max(r["quality_boost"] for r in low_quality_results)

                # Quality boost should be higher for high quality doc
                assert high_quality_boost > low_quality_boost, (
                    f"High quality boost ({high_quality_boost}) should be higher than low quality boost ({low_quality_boost})"
                )

                # If we have meaningful search scores, high quality should score higher
                if (
                    high_score > 0.01 or low_score > 0.01
                ):  # If we have meaningful scores
                    assert high_score > low_score, (
                        f"High quality document should score higher: {high_score} vs {low_score}"
                    )
                else:
                    # If scores are very low/zero (no embeddings), just verify quality boosting worked
                    print(
                        "Note: Low search scores detected (likely no PyTorch/embeddings), verifying quality boost only"
                    )
    ...
    async def test_token_counting(self, embedding_service, sample_document):
        ...
        for chunk_uuid in chunk_uuids:
            ...
            assert token_count >= word_count, (
                "Token count should be at least word count"
            )
    ...
    async def test_delete_document_chunks(self, embedding_service, sample_document):
        ...
        # Delete chunks
        deleted_count = await embedding_service.delete_document_chunks(
            sample_document.uid
        )

        assert deleted_count == len(chunk_uuids), (
            "Should delete all chunks for document"
        )
