...
class TestOpenAIEmbeddings:
    ...
    async def test_collection(self, weaviate_client):
        ...
        try:
            ...
            assert success, "Failed to create collection - check if text2vec-openai module is available in Weaviate"
    ...
    async def test_embedding_generation(self, test_collection, test_config):
        ...
        vector_data = chunk.vector.get('default') if isinstance(chunk.vector, dict) else chunk.vector
        ...
        assert actual_dims == 1536, f"Vector should have 1536 dimensions, got {actual_dims}"
        assert all(isinstance(x, float) for x in vector_data), "Vector should contain floats"
    ...
    async def test_semantic_vs_bm25_search(self, test_collection, test_config):
        ...
        semantic_medical_results = [
            ...
            if any(word in r["text"].lower() for word in ["hyperglycemia", "blood sugar", "glucose", "insulin"])
        ]
        ...
        print(f"Semantic results: {len(semantic_results)}, BM25 results: {len(bm25_results)}")
        
        # At minimum, semantic should find medical content even without exact matches
        assert len(semantic_medical_results) >= 1, f"Semantic search should find medical content, got: {[r['title'] for r in semantic_results]}"
        ...
        print(f"Found {len(semantic_medical_results)} medical results via semantic search")
    ...
    async def test_embedding_consistency(self, test_collection, test_config):
        ...
        doc1 = Document(uid="test:consistency1", source="test", source_id="consistency1", text=text_content)
        doc2 = Document(uid="test:consistency2", source="test", source_id="consistency2", text=text_content)
        ...
        assert cosine_similarity > 0.98, f"Embeddings should be nearly identical, similarity: {cosine_similarity}"
    ...
    async def test_quality_boosting_with_openai(self, test_collection, test_config):
        ...
        high_quality_results = [r for r in results if "randomized controlled trial" in r["text"].lower()]
        low_quality_results = [r for r in results if "personal blog" in r["text"].lower()]
        ...
        if len(high_quality_results) > 0 and len(low_quality_results) > 0:
            ...
            print(f"Quality boosting working: high={high_score:.3f}, low={low_score:.3f}")
        else:
            print("Quality boosting test inconclusive - not all documents found in results")
    ...
    async def test_hybrid_search_alpha_weighting(self, test_collection, test_config):
        ...
        assert len(semantic_weighted) > 0, "Semantic-weighted search should find results"
        
        # The exact match should rank higher in BM25-weighted search
        if len(bm25_weighted) > 1:
            top_bm25_result = bm25_weighted[0]
            assert "exact word oncology" in top_bm25_result["text"].lower(), "BM25-weighted should favor exact match"
    ...
    async def test_openai_api_fallback(self, weaviate_client):
        ...
        try:
            ...
            # Store a document
            doc = Document(uid="test:fallback", source="test", source_id="fallback", text="fallback test content")
            chunk_ids = await service.store_document_chunks(doc)
            assert len(chunk_ids) > 0
...
class TestOpenAIScaleFeatures:
    ...
    async def test_complex_biomedical_embedding(self, weaviate_client):
        ...
        try:
            ...
            actual_dims = len(vector_data)
            assert actual_dims == 1536, f"Should get 1536 dimensions with text-embedding-3-small, got {actual_dims}"
    ...
    async def test_large_corpus_embedding(self, weaviate_client):
        ...
        try:
            ...
            biomedical_topics = [
                ...
                "cardiovascular", "immunology", "pharmacology", "genetics", "neurology"
            ]
            ...
            print(f"Storing {len(documents)} documents (this will make multiple OpenAI API calls)...")
            ...
            assert total_chunks >= len(documents), f"Should generate at least {len(documents)} chunks, got {total_chunks}"
            ...
            diabetes_results = [r for r in search_results if "diabetes" in r["text"].lower()]
            assert len(diabetes_results) > 0, "Should find diabetes-related content via semantic search"
            
            print(f"✓ Large corpus test complete: {total_chunks} chunks, {len(search_results)} search results")
