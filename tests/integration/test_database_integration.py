...
class BiomedicDataGenerator:
    ...
    RESEARCH_TOPICS: ClassVar[list[str]] = [
        "cancer immunotherapy", "CRISPR gene editing", "Alzheimer's disease", "COVID-19 therapeutics",
        "personalized medicine", "stem cell therapy", "precision oncology", "biomarker discovery",
        "drug resistance", "clinical trials", "genomic medicine", "regenerative medicine",
        "targeted therapy", "diagnostic imaging", "pharmacogenomics", "rare diseases"
    ]
    
    # High-impact journals
    JOURNALS: ClassVar[list[str]] = [
        "Nature", "Science", "Cell", "New England Journal of Medicine", "The Lancet",
        "Nature Medicine", "Cell Medicine", "Nature Genetics", "Nature Biotechnology",
        "Cancer Research", "Blood", "Journal of Clinical Oncology", "Clinical Cancer Research",
        "Nature Immunology", "Science Translational Medicine", "PLoS Medicine"
    ]
    
    # Common MeSH terms for biomedical research
    MESH_TERMS: ClassVar[list[str]] = [
        "Neoplasms", "Immunotherapy", "Biomarkers", "Gene Expression", "Clinical Trials",
        "Drug Therapy", "Therapeutics", "Precision Medicine", "Genomics", "Proteomics",
        "Pharmacology", "Oncology", "Immunology", "Molecular Biology", "Cell Biology"
    ]
    
    # Research keywords
    KEYWORDS: ClassVar[list[str]] = [
        "biomarker", "therapeutic target", "clinical trial", "drug discovery", "gene expression",
        "protein interaction", "molecular mechanism", "pathway analysis", "personalized treatment",
        "precision medicine", "immunotherapy", "checkpoint inhibitor", "CAR-T", "monoclonal antibody"
    ]
    
    # Author name patterns
    FIRST_NAMES: ClassVar[list[str]] = ["John", "Mary", "David", "Sarah", "Michael", "Jennifer", "Robert", "Lisa", "James", "Maria"]
    ...
    @classmethod
    def generate_document_data(cls, pmid: str | None = None, topic: str | None = None) -> dict[str, Any]:
        ...
        return {
            ...
            "keywords": cls._random_sample(cls.KEYWORDS, k=cls._random_choice([3, 4, 5]))
        }
    
    @classmethod
    def generate_corpus_checkpoint_data(cls, checkpoint_id: str | None = None) -> dict[str, Any]:
...
class TestDatabaseManagerDocuments:
    ...
    async def test_create_document_minimal_required(self, database_manager):
        ...
        doc_data = {
            "pmid": pmid,
            "title": "Minimal Test Document for Cancer Research"
        }
    ...
    async def test_list_documents_with_data(self, database_manager):
        ...
        doc_data_list = [
            BiomedicDataGenerator.generate_document_data() 
            for _ in range(5)
        ]
...
@pytest.mark.skip(reason="Database implementation has JSON serialization bug in corpus checkpoints - needs fixing")
class TestDatabaseManagerCorpusCheckpoints:
    ...
    async def test_create_checkpoint_validation_errors(self, database_manager):
        ...
        with pytest.raises(ValidationError):
            await manager.create_corpus_checkpoint(
                checkpoint_id="",
                name="Valid Name"
            )
    ...
    async def test_list_corpus_checkpoints(self, database_manager):
        ...
        checkpoints_data = [
            BiomedicDataGenerator.generate_corpus_checkpoint_data()
            for _ in range(3)
        ]
