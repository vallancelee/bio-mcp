...
async def populated_weaviate():
    ...
    try:
        ...
        for i, (doc, quality_score) in enumerate(zip(test_documents, quality_scores), 1):
            try:
                chunk_uuids = await service.store_document_chunks(doc, quality_score=quality_score)
                total_chunks += len(chunk_uuids)
                stored_uids.append(doc.uid)
                print(f"  ✓ Added document {i}/{len(test_documents)}: {len(chunk_uuids)} chunks")
