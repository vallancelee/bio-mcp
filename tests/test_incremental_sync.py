...
class TestIncrementalPubMedSearch:
    ...
    async def test_incremental_search_with_edat_filter(self):
        ...
        with patch(
            "src.bio_mcp.clients.pubmed_client.PubMedClient"
            ...
            result = await mock_client.search_incremental(
                query="diabetes treatment", last_edat="2024/08/15", limit=50
            )
    ...
    async def test_incremental_search_without_edat_filter(self):
        ...
        with patch(
            ...
            result = await mock_client.search_incremental(
                query="cancer immunotherapy", last_edat=None, limit=10
            )
...
class TestIncrementalSyncOrchestration:
    ...
    async def test_incremental_sync_with_existing_watermark(self):
        ...
        with patch(
            "src.bio_mcp.services.services.PubMedService"
        ) as mock_pubmed_service_class:
            with patch(
                "src.bio_mcp.services.services.DocumentService"
            ) as mock_doc_service_class:
                with patch(
                    "src.bio_mcp.services.services.VectorService"
                    ...
                    mock_doc_service.manager.get_sync_watermark.return_value = (
                        mock_watermark
                    )
                    ...
                    mock_pubmed_service.client.search_incremental.return_value = (
                        mock_search_result
                    )

                    # Mock document existence checks
                    mock_doc_service.document_exists.side_effect = [
                        False,
                        False,
                    ]  # Both are new
                    ...
                    orchestrator = SyncOrchestrator(
                        mock_pubmed_service, mock_doc_service, mock_vector_service
                    )
                    result = await orchestrator.sync_documents_incremental(
                        "heart disease", limit=50
                    )
    ...
    @pytest.mark.asyncio
    @pytest.mark.skip(
        reason="Requires Weaviate - needs better mocking or testcontainers"
    )
    async def test_incremental_sync_no_new_documents(self):
        ...
        with patch(
            ...
            with patch(
                ...
                orchestrator = SyncOrchestrator(
                    mock_pubmed_service, mock_doc_service, None
                )
                result = await orchestrator.sync_documents_incremental(
                    "rare disease", limit=50
                )
...
class TestIncrementalSyncMCPTool:
    ...
    async def test_incremental_sync_tool_success(self):
        ...
        with patch(
            "src.bio_mcp.mcp.pubmed_tools.get_tools_manager"
        ) as mock_get_manager:
            ...
            result = await pubmed_sync_incremental_tool(
                "pubmed.sync.incremental", {"query": "alzheimer disease", "limit": 50}
            )
            ...
            mock_manager.sync_incremental.assert_called_once_with(
                "alzheimer disease", limit=50
            )
    ...
    async def test_incremental_sync_tool_missing_query(self):
        ...
        result = await pubmed_sync_incremental_tool(
            "pubmed.sync.incremental", {"limit": 100}
        )
