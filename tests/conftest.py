...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "testcontainers: marks tests that use TestContainers"
    )
...
def pytest_collection_modifyitems(config, items):
    ...
    for item in items:
        ...
        # Add testcontainers marker to testcontainers-related tests
        if (
            "testcontainers" in item.name.lower()
            or "testcontainers" in str(item.fspath).lower()
            or "tests/integration/database" in str(item.fspath)
        ):
