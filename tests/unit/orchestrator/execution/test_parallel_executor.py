...
class TestParallelExecutor:
    ...
    async def test_execute_multiple_tasks_parallel(self):
        ...
        assert {result.data["result"] for result in results} == {
            "processed_task1", "processed_task2", "processed_task3"
        }
    ...
    async def test_rate_limiting_applied(self):
        ...
        rate_limiter = TokenBucketRateLimiter(capacity=1, refill_rate=2.0)  # 2 tokens per second
        executor = ParallelExecutor(rate_limiter=rate_limiter, max_concurrency=5)
        ...
        execution_time = end_time - start_time
        assert execution_time >= 0.5  # At least some significant delay due to rate limiting
    ...
    async def test_concurrency_limiting(self):
        ...
        rate_limiter = TokenBucketRateLimiter(capacity=10, refill_rate=20.0)  # Very permissive
        ...
        tasks = [
            {"func": test_task, "args": (f"task{i}",), "kwargs": {}}
            for i in range(5)
        ]
    ...
    async def test_error_handling(self):
        ...
        async def success_task() -> NodeResult:
            return NodeResult(success=True, data={"status": "ok"}, node_name="success_task")
    ...
    async def test_execute_with_timeout(self):
        ...
        async def slow_task() -> NodeResult:
            ...
            return NodeResult(success=True, data={"result": "slow"}, node_name="slow_task")
            
        async def fast_task() -> NodeResult:
            await asyncio.sleep(0.01)
            return NodeResult(success=True, data={"result": "fast"}, node_name="fast_task")
    ...
    async def test_custom_token_cost(self):
        ...
        async def expensive_task() -> NodeResult:
            return NodeResult(success=True, data={"cost": "high"}, node_name="expensive_task")
            
        async def cheap_task() -> NodeResult:
            return NodeResult(success=True, data={"cost": "low"}, node_name="cheap_task")
