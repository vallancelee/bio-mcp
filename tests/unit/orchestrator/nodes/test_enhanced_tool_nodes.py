...
class TestEnhancedPubMedNode:
    ...
    async def test_enhanced_pubmed_search_with_topic(self):
        ...
        # Mock the executor
        node.executor.execute_parallel = AsyncMock(return_value=[
            NodeResult(
                success=True,
                data={"results": [{"pmid": "12345", "title": "Test Article"}], "total": 1},
                cache_hit=False,
                latency_ms=100.0,
    ...
    async def test_enhanced_pubmed_multiple_search_terms(self):
        ...
        node.executor.execute_parallel = AsyncMock(return_value=[
            NodeResult(
                success=True,
                data={"results": [{"pmid": "12345", "title": "Diabetes Article"}], "total": 1},
                cache_hit=False,
                latency_ms=100.0,
                node_name="pubmed.search"
            ),
            NodeResult(
                success=True,
                data={"results": [{"pmid": "67890", "title": "Pharma Article"}], "total": 1},
                cache_hit=True,
                latency_ms=50.0,
                node_name="pubmed.search"
            )
        ...
        assert set(result["pubmed_results"]["search_terms"]) == {"diabetes", "Pfizer[AD]"}
    ...
    async def test_enhanced_pubmed_deduplication(self):
        ...
        node.executor.execute_parallel = AsyncMock(return_value=[
            ...
            NodeResult(
                success=True,
                data={"results": [
                    {"pmid": "12345", "title": "Article 1 Duplicate"},  # Same PMID
        ...
        pmids = {article["pmid"] for article in result["pubmed_results"]["search_results"]}
...
class TestEnhancedTrialsNode:
    ...
    async def test_enhanced_trials_search_with_condition(self):
        ...
        node.executor.execute_parallel = AsyncMock(return_value=[
            NodeResult(
                success=True,
                data={"results": [{"nct_id": "NCT12345", "title": "Diabetes Trial"}], "total": 1},
                cache_hit=False,
                latency_ms=150.0,
    ...
    async def test_enhanced_trials_company_search(self):
        ...
        node.executor.execute_parallel = AsyncMock(return_value=[
            NodeResult(
                success=True,
                data={"results": [{"nct_id": "NCT67890", "title": "Pfizer Trial"}], "total": 1},
                cache_hit=False,
                latency_ms=120.0,
                node_name="clinicaltrials.search"
            )
