...
class TestErrorClassifier:
    ...
    def test_classify_by_node_name(self):
        ...
        generic_error = NodeResult(
            success=False, error_message="Connection failed", node_name="pubmed_search"
        )

        error_type, severity = classifier.classify_error(generic_error)
        assert (
            error_type == ErrorType.NETWORK_TIMEOUT
        )  # Connection issues treated as network

        # Test with different node
        frame_error = NodeResult(
            success=False, error_message="Connection failed", node_name="frame_parser"
        )
...
class TestErrorRecoveryManager:
    ...
    async def test_create_recovery_strategy_rate_limit(self):
        ...
        strategy = await manager.create_recovery_strategy(
            error_result, attempt=1, max_attempts=3
        )
    ...
    async def test_create_recovery_strategy_network_timeout(self):
        ...
        error_result = NodeResult(
            success=False, error_message="Request timeout", node_name="trials_search"
        )

        strategy = await manager.create_recovery_strategy(
            error_result, attempt=2, max_attempts=3
        )
    ...
    async def test_create_recovery_strategy_authentication(self):
        ...
        error_result = NodeResult(
            success=False, error_message="Invalid API key", node_name="pubmed_search"
        )
    ...
    async def test_create_recovery_strategy_max_retries(self):
        ...
        strategy = await manager.create_recovery_strategy(
            error_result, attempt=3, max_attempts=3
        )
    ...
    async def test_execute_recovery_skip_and_continue(self):
        ...
        node_result = NodeResult(
            success=False, error_message="Invalid parameter", node_name="pubmed_search"
        )

        recovered_state = await manager.execute_recovery(
            strategy, state, node_result, "pubmed_search"
        )
    ...
    async def test_execute_recovery_partial_results(self):
        ...
        node_result = NodeResult(
            success=False, error_message="Connection timeout", node_name="trials_search"
        )

        recovered_state = await manager.execute_recovery(
            strategy, state, node_result, "trials_search"
        )
    ...
    def test_should_retry_logic(self):
        ...
        assert (
            manager._should_retry(ErrorType.RATE_LIMIT, ErrorSeverity.RECOVERABLE, 1, 3)
            is True
        )
        assert (
            manager._should_retry(
                ErrorType.NETWORK_TIMEOUT, ErrorSeverity.RECOVERABLE, 2, 3
            )
            is True
        )
        ...
        assert (
            manager._should_retry(
                ErrorType.AUTHENTICATION, ErrorSeverity.CRITICAL, 1, 3
            )
            is False
        )
        ...
        assert (
            manager._should_retry(ErrorType.RATE_LIMIT, ErrorSeverity.RECOVERABLE, 3, 3)
            is False
        )
        assert (
            manager._should_retry(
                ErrorType.NETWORK_TIMEOUT, ErrorSeverity.RECOVERABLE, 4, 3
            )
            is False
        )
        ...
        assert (
            manager._should_retry(
                ErrorType.VALIDATION, ErrorSeverity.NON_RECOVERABLE, 1, 3
            )
            is False
        )
