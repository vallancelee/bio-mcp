...
class TestUniversalDocument:
    ...
    def test_model_creation(self):
        ...
        doc = UniversalDocument(
            id="pubmed:12345", source="pubmed", source_id="12345", title="Test Document"
        )
...
class TestSyncWatermark:
    ...
    def test_model_creation(self):
        ...
        watermark = SyncWatermark(
            source="pubmed", query_key="cancer_biomarkers", last_sync=timestamp
        )
    ...
    def test_different_sources(self):
        ...
        pubmed_wm = SyncWatermark(
            source="pubmed", query_key="immunotherapy", last_sync=timestamp
        )

        ct_wm = SyncWatermark(
            source="clinicaltrials", query_key="immunotherapy", last_sync=timestamp
        )
    ...
    def test_table_args_configuration(self):
        ...
        if isinstance(SyncWatermark.__table_args__, tuple):
            table_config = (
                SyncWatermark.__table_args__[-1] if SyncWatermark.__table_args__ else {}
            )
        else:
...
class TestCorpusCheckpoint:
    ...
    def test_model_creation(self):
        ...
        assert (
            ...
            == "Comprehensive cancer research corpus including latest trials"
        )
    ...
    def test_checkpoint_with_parent(self):
        ...
        assert checkpoint.primary_queries == [
            "cancer",
            "immunotherapy",
            "clinical trial",
        ]
    ...
    async def test_model_relationships(self):
        ...
        with PostgresContainer("postgres:13") as postgres:
            ...
            try:
                ...
                doc = UniversalDocument(
                    id="test:123", source="test", source_id="123", title="Test Document"
                )

                watermark = SyncWatermark(
                    source="test", query_key="test_query", last_sync=datetime.now()
                )

                checkpoint = CorpusCheckpoint(
                    checkpoint_id="test_checkpoint", name="Test Checkpoint"
                )
                ...
                doc_count = await session.scalar(
                    select(func.count(UniversalDocument.id))
                )
                watermark_count = await session.scalar(
                    select(func.count(SyncWatermark.id))
                )
                checkpoint_count = await session.scalar(
                    select(func.count(CorpusCheckpoint.checkpoint_id))
                )
