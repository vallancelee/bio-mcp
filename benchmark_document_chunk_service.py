...
class DocumentChunkBenchmark:
    ...
    def __init__(self):
        ...
        # Use test collection
        self.collection_name = "DocumentChunk_benchmark_" + datetime.now().strftime("%Y%m%d_%H%M%S")
    ...
    async def benchmark_search(self, queries):
        ...
        for i, query in enumerate(queries):
            ...
            results = await self.service.search_chunks(query, limit=10, search_mode="hybrid")
            query_time = time.time() - query_start
            print(f"  Query {i+1}: '{query}' -> {len(results)} results in {query_time:.3f}s")
