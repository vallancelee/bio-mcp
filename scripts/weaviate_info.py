...
def get_collection_info(collection_name: str | None = None) -> dict:
    ...
    try:
        ...
        if collection_name:
            ...
            result = {"collection": info, "validation": validation}
        else:
...
def main():
    ...
    parser.add_argument(
        "--validate", action="store_true", help="Validate collection schema"
    )
    ...
    if args.json:
        ...
        if args.collection:
            ...
            if collection_info["exists"]:
                ...
                if args.validate:
                    print(
                        f"   Schema Valid: {'✅' if validation_info['valid'] else '❌'}"
                    )
                    ...
                    print(
                        f"   Properties Found: {len(validation_info['properties_found'])}"
                    )
            ...
            for name, collection_info in info.get("collections", {}).items():
                ...
                docs = (
                    f"{collection_info.get('total_documents', 0):,}"
                    if collection_info["exists"]
                    else "N/A"
                )
