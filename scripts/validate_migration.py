...
async def validate_chunk_counts(
    document_chunk_service: DocumentChunkService, db_service: DatabaseService
) -> dict[str, Any]:
    ...
    return {
        ...
        "avg_chunks_per_doc": (
            collection_stats.get("total_chunks", 0) / db_stats.get("total_documents", 1)
        ),
    }


async def validate_sample_documents(
    document_chunk_service: DocumentChunkService, sample_size: int = 10
) -> dict[str, Any]:
...
@click.option(
    ...
    type=int,
    default=100,
    help="Number of documents to sample for validation",
)
def main(sample_size):
    ...
    async def _main():
        ...
        try:
            ...
            count_validation = await validate_chunk_counts(
                document_chunk_service, db_service
            )
            ...
            click.echo(
                f"Average chunks per document: {count_validation['avg_chunks_per_doc']:.1f}"
            )

            click.echo(f"\nValidating sample of {sample_size} documents...")
            sample_validation = await validate_sample_documents(
                document_chunk_service, sample_size
            )
