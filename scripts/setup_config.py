...
def main():
    ...
    print()
    pubmed_key = input("NCBI/PubMed API Key (optional, for enhanced rate limits): ").strip()
    if pubmed_key:
        config_content = config_content.replace("your-ncbi-api-key-here", pubmed_key)
    
    # Custom Weaviate URL (optional)
    print()
    weaviate_url = input("Weaviate URL (optional, default: http://localhost:8080): ").strip()
    if weaviate_url:
    ...
    print("   uv run python clients/cli.py pubmed.sync --query \"diabetes treatment\" --limit 3")
    print("   uv run python clients/cli.py rag.search --query \"diabetes management\" --top-k 3")
