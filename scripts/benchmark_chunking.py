...
def generate_synthetic_documents(count: int, size: str = "medium") -> list[Document]:
    ...
    base_texts = {
        ...
        "medium": "This is a medium-sized document for performance testing. "
        * 100,  # ~600 words
        "large": "This is a large document for comprehensive performance testing. "
        * 500,  # ~3000 words
        "xlarge": "This is an extra large document for stress testing performance. "
        * 1000,  # ~6000 words
    }
...
def benchmark_chunking_speed(
    documents: list[Document], config: ChunkingConfig, iterations: int = 3
) -> dict[str, Any]:
    ...
    for i in range(iterations):
        ...
        print(
            f"      Iteration {i + 1}: {elapsed:.3f}s, {total_chunks} chunks, {end_memory - start_memory:.1f}MB memory delta"
        )
...
def benchmark_configurations(iterations: int = 3) -> dict[str, Any]:
    ...
    configs = {
        "small_chunks": ChunkingConfig(
            target_tokens=150, max_tokens=200, overlap_tokens=25
        ),
        "default": ChunkingConfig(),
        "large_chunks": ChunkingConfig(
            target_tokens=450, max_tokens=600, overlap_tokens=75
        ),
    ...
    for config_name, config in configs.items():
        ...
        print(
            f"      Target/Max: {config.target_tokens}/{config.max_tokens}, Overlap: {config.overlap_tokens}"
        )
...
def print_benchmark_summary(
    ...
    for count, results in scalability_results.items():
        ...
        print(
            f"   {count:3d} docs: {results['avg_time']:6.3f}s ({efficiency:5.1f} docs/sec, {stability:4.1f}% variance)"
        )
    ...
    for config_name, results in config_results.items():
        ...
        print(
            f"   {config_name:12s}: {results['avg_time']:6.3f}s ({chunks_per_doc:4.1f} chunks/doc, {results['chunks_per_second']:5.1f} chunks/sec)"
        )
    ...
    large_doc_time = (
        size_results.get("large", {}).get("avg_time", 0) / 50
    )  # Per document
    print(
        f"   Large doc processing: {large_doc_time:.3f}s per doc (target: <1.0s) {'✅' if large_doc_time < 1.0 else '❌'}"
    )
    ...
    print(
        f"   UUID generation: {uuid_time_per_chunk:.3f}ms per chunk (target: <0.1ms) {'✅' if uuid_time_per_chunk < 0.1 else '❌'}"
    )
    ...
    print(
        f"   • Fastest config: {fastest_config[0]} ({fastest_config[1]['avg_time']:.3f}s)"
    )

    most_efficient = max(
        config_results.items(), key=lambda x: x[1]["chunks_per_second"]
    )
    print(
        f"   • Most efficient: {most_efficient[0]} ({most_efficient[1]['chunks_per_second']:.1f} chunks/sec)"
    )

    # Memory and stability notes
    largest_test = max(scalability_results.keys())
    stability = (
        scalability_results[largest_test]["stdev_time"]
        / scalability_results[largest_test]["avg_time"]
    )
    if stability < 0.1:
        print(f"   • Performance is stable (variance: {stability * 100:.1f}%)")
    else:
        print(
            f"   • Performance shows some variance (variance: {stability * 100:.1f}%)"
        )
...
def main():
    ...
    parser.add_argument(
        "--skip-scalability", action="store_true", help="Skip scalability tests"
    )
    parser.add_argument(
        "--skip-sizes", action="store_true", help="Skip document size tests"
    )
    parser.add_argument(
        "--skip-configs", action="store_true", help="Skip configuration tests"
    )
    ...
    try:
        ...
        if all(key in results for key in ["scalability", "sizes", "configs"]):
            print_benchmark_summary(
                results["scalability"], results["sizes"], results["configs"]
            )
        else:
