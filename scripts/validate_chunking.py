...
Conclusions: COVID-19 vaccines demonstrate high effectiveness against severe disease and moderate effectiveness against infection. Booster doses are important for maintaining protection.""",
        ...
        Document(
            ...
            detail={
                ...
                "mesh_terms": ["Machine Learning", "Drug Discovery", "Artificial Intelligence"]
            }
        ),
...
Conclusions: This comprehensive genomic analysis provides insights into the molecular basis of cancer and identifies potential targets for therapeutic intervention. The high frequency of TP53 mutations highlights the importance of targeting p53 pathway dysfunction in cancer treatment.""",
            ...
            detail={
                ...
                "mesh_terms": ["Cancer", "Genomics", "Personalized Medicine", "Machine Learning"]
            }
        )
    ]
...
def validate_chunking_quality(chunks: list[Any], document: Document, config: ChunkingConfig) -> dict[str, Any]:
    ...
    for i, chunk in enumerate(chunks):
        ...
        if chunk.tokens > config.max_tokens:
            ...
            results["issues"].append(f"Chunk {i}: Exceeds max tokens ({chunk.tokens} > {config.max_tokens})")
    ...
    if any(section in document.text for section in ["Background:", "Methods:", "Results:", "Conclusions:"]):
        section_chunks = [c for c in chunks if c.section != "Unstructured"]
        if section_chunks:
            results["section_detection"] = f"Detected {len(set(c.section for c in section_chunks))} sections"
        else:
    ...
    for chunk in chunks:
        ...
        if re.search(r'\d+\.\d+%.*\(.*p[<=]', chunk.text):
            ...
            if not re.search(r'p[<=]\d+\.\d+', chunk.text):
                ...
                results["issues"].append(f"Potential incomplete statistical claim in chunk {chunk.chunk_id}")
...
def run_validation(sample_size: int | None = None) -> dict[str, Any]:
    ...
    for config_name, config in configs:
        ...
        print(f"   Target tokens: {config.target_tokens}, Max tokens: {config.max_tokens}")
        ...
        for doc in documents:
            ...
            # Print basic stats
            print(f"      → {len(chunks)} chunks, {validation['avg_tokens']:.1f} avg tokens")
        ...
        validation_results[config_name] = {
            ...
            "avg_chunks_per_doc": sum(r["chunk_count"] for r in config_results) / len(config_results),
            "avg_tokens_per_chunk": sum(r["avg_tokens"] for r in config_results) / len(config_results),
            "compliance_rate": sum(1 for r in config_results if r["token_budget_compliance"] and not r["issues"]) / len(config_results)
        }
        
        print(f"   ⏱️  Processing time: {end_time - start_time:.2f}s")
        print(f"   ✅ Compliance rate: {validation_results[config_name]['compliance_rate']:.1%}")
...
def print_summary(results: dict[str, Any]):
    ...
    for config_name, config_results in results.items():
        ...
        print(f"   Target/Max tokens: {config_results['config']['target_tokens']}/{config_results['config']['max_tokens']}")
    ...
    print(f"   • Best performing config: {best_config[0]} ({best_config[1]['compliance_rate']:.1%} compliance)")
    
    # Performance recommendations
    fastest_config = min(results.items(), key=lambda x: x[1]["processing_time"])
    print(f"   • Fastest processing: {fastest_config[0]} ({fastest_config[1]['processing_time']:.2f}s)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Validate chunking strategy on sample data")
    parser.add_argument("--sample-size", type=int, default=None, help="Number of sample documents to test")
