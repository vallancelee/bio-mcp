...
bio_mcp_client: Optional[BioMCPClient] = None
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        raise RuntimeError(f"POC backend startup failed: LangGraph orchestrator initialization error: {e}") from e
...
async def root():
    ...
    return {
        ...
        "bio_mcp_connected": bio_mcp_client is not None and await bio_mcp_client.health_check()
    }
    ...
    async def generate_events():
        ...
        if query_state["status"] == QueryStatus.COMPLETED:
            ...
            final_event = {
                ...
                "total_results": sum(
                    r.get("total_found", 0) for r in query_state["results"].values()
...
async def get_active_queries():
    ...
    for query_id, state in active_queries.items():
        active_query_list.append({
            ...
            "total_results": sum(
                r.get("total_found", 0) for r in state.get("results", {}).values()
                if isinstance(r, dict)
            ) if state.get("results") else 0
        })
...
async def process_pubmed_search(query_id: str):
    ...
    try:
        ...
        results = await bio_mcp_client.pubmed_search(
            query=query_state["query"],
            max_results=query_state["max_results"]
        )
        ...
        logger.info(f"PubMed search completed for query {query_id}: {len(results.get('results', []))} results")
...
async def process_clinical_trials_search(query_id: str):
    ...
    try:
        ...
        logger.info(f"Clinical trials search completed for query {query_id}: {len(results.get('studies', []))} results")
...
async def process_rag_search(query_id: str):
    ...
    try:
        ...
        logger.info(f"RAG search completed for query {query_id}: {len(results.get('documents', []))} results")
