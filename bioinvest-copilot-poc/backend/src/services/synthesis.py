...
class SynthesisService:
    ...
    async def synthesize_results(
        ...
        try:
            ...
            synthesis = await self._generate_synthesis_simulation(query, results, options)
    ...
    async def _generate_synthesis_simulation(
        ...
        if any(term in query_lower for term in ["glp-1", "semaglutide", "novo nordisk"]):
            key_insights = [
                {
        ...
        # Clinical trial prediction queries
        elif any(term in query_lower for term in ["predict", "success", "probability", "trial"]):
            key_insights = [
                {
            ...
            summary = f"""Analysis of similar clinical trials suggests moderate success probability. 
    ...
    def _calculate_quality_metrics(
        ...
        source_diversity = len([k for k, v in results.items() if v])  # Number of sources with results
        
        completeness = min(0.95, 0.5 + (total_results * 0.02))  # More results = higher completeness
        ...
        relevance = sum(insight.get("confidence", 0.5) for insight in insights) / max(len(insights), 1)
        
        overall_score = (completeness * 0.25 + recency * 0.20 + authority * 0.20 + 
                        diversity * 0.15 + relevance * 0.20)
    ...
    def _extract_citations(self, results: Dict[str, List[Dict[str, Any]]]) -> List[Dict[str, Any]]:
        ...
        for article in results.get("pubmed", []):
            citations.append({
                ...
                "year": self._extract_year_from_date(article.get("publication_date", "")),
        ...
        for trial in results.get("clinical_trials", []):
            citations.append({
                ...
                "year": self._extract_year_from_date(trial.get("dates", {}).get("start_date", "")),
    ...
    def _generate_fallback_synthesis(
        ...
        sources_summary = {source: len(source_results) for source, source_results in results.items()}
            ...
            "key_insights": [
                {
                    ...
                    "supporting_evidence": [f"Found results in {len(sources_summary)} data sources"],
                    "confidence": 0.6,
