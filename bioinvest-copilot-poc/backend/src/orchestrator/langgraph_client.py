...
class LangGraphOrchestrator:
    ...
    async def initialize(self):
        ...
        try:
            ...
            logger.info("LangGraph orchestrator initialized successfully with async checkpointing")
    ...
    def __del__(self):
        ...
        if self.checkpointer_ctx:
            logger.warning("LangGraph orchestrator destroyed without proper async cleanup")
                    ...
                    if state:
                        ...
                        await stream_callback({
                            ...
                            "data": {
                                "node": current_node,
                                "step": node_count,
                                "results_preview": self._get_results_preview(final_state),
                                "timestamp": datetime.now(UTC).isoformat()
                            }
                        })
            else:
        ...
        if current_node in ["pubmed_search", "ctgov_search", "rag_search"]:
            ...
            if current_node in field_mapping:
                ...
                if source_data:
                    ...
                    logger.info(f"Captured {source_key} results: {type(source_data)} with keys {list(source_data.keys()) if isinstance(source_data, dict) else 'Not a dict'}")
                else:
                    logger.warning(f"No results found in state field '{state_field}' for {current_node}")
    ...
    async def _format_langgraph_results(
        ...
        if isinstance(final_state, dict):
            ...
            for key, value in final_state.items():
                if key == "answer" and value:
                    ...
                    if isinstance(value, str):
                        logger.info(f"  answer content (first 200 chars): {value[:200]}...")
                    elif isinstance(value, dict):
                        logger.info(f"  answer keys: {list(value.keys())}")
                elif key == "messages":
                    logger.info(f"  {key}: {len(value) if isinstance(value, list) else type(value).__name__} messages")
            ...
            logger.info(f"Final state attributes: {dir(final_state) if hasattr(final_state, '__dict__') else 'No dir available'}")
        ...
        logger.info(f"Using captured sources: {list(raw_sources.keys()) if raw_sources else 'none captured'}")
        ...
        for source_name, source_data in raw_sources.items():
            if isinstance(source_data, dict) and 'results' in source_data:
                ...
                logger.info(f"Transformed {source_name}: {len(source_data['results'])} results")
            else:
                # Fallback for unexpected structure
                logger.warning(f"Unexpected source data structure for {source_name}: {type(source_data)}")
                sources[source_name] = source_data
        
        # Fallback to final state extraction if no sources were captured
        if not sources:
            logger.warning("No sources captured during execution, attempting final state extraction")
        ...
        if not synthesis:
            ...
            synthesis = await self._generate_synthesis_from_langgraph(final_state, sources)
        ...
        metadata = {
            ...
            "tool_calls_made": self._safe_get_state_field(final_state, "tool_calls_made", []),
        ...
        logger.info(f"LangGraph returning sources: {list(sources.keys()) if sources else 'empty'}")
        if sources:
            for source, data in sources.items():
                if isinstance(data, dict):
                    logger.info(f"  {source}: keys={list(data.keys())}, has results={bool(data.get('results', []))}")
                else:
    ...
    async def _generate_synthesis_from_langgraph(
        ...
        logger.info(f"Using query text for synthesis: {query_text[:100] if query_text else 'None'}")
        ...
        synthesis["generation_metadata"]["execution_path"] = self._reconstruct_execution_path(state)
        synthesis["generation_metadata"]["tool_calls"] = len(state.get("tool_calls_made", []))
